...
(function ($) {
	...
	$(document).ready(function () {
		...
		var a = $("#datatable-buttons").DataTable({
			lengthChange: !1,
			buttons: ["copy", "print"],
			language: {paginate: {previous: "<i class='fa fa-chevron-left'>", next: "<i class='fa fa-chevron-right'>"}},
			drawCallback: function () {
				$(".dataTables_paginate > .pagination").addClass("pagination-rounded")
			}
		});
...
// Ensure FullCalendar is loaded
		...
		try {
			var calendar = new FullCalendar.Calendar($calendar[0], {
				...
				// Event source with error handling
				events: {
					url: 'calendar.php?get_events=1',
					method: 'GET',
				...
				loading: function(isLoading) {
					$calendar.toggleClass('fc-loading', isLoading);
				},

				// Error handling for event source
				eventSourceFailure: function(error) {
					console.error('Error loading events:', error);
					alert('Failed to load events. Please refresh the page or try again later.');
				}
			});
			...
			$calendar.html('Error initializing calendar. Please refresh the page.');
		}

		setInterval(function() {
			$('.alert:not(.alert-warning)').fadeOut(400, function() {
				$(this).remove();
			});
		}, 5000);
